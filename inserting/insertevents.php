...
function my_insert_posts($slice) {
    ...
    wp_set_object_terms( $original_post_id, $slice['eventtypefrench'], 'event_type' );
    wp_set_object_terms( $translated_post_id, $slice['eventtypeenglish'], 'event_type' );
...
function element_connect_on_insert($slice) {
    ...
    $slices = json_decode(file_get_contents('realevents.json'), true);
    if ($slices) { 
